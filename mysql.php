...
function ins($data1,$data2,$data3){
	$sql= "SELECT * FROM `codednf` WHERE codekey='$data1' and namer='1';";    //根据 Id值进行查询
    $stmt = link_mysql($sql);
    if($stmt->execute()) {
		$arry_data = $stmt->fetch(PDO::FETCH_ASSOC);
			if($arry_data['codekey']!=""){
				up($data2,$data1);
				return json_encode(array('msg'=>'您的预约成功，将于7月28日以邮件方式提醒您登陆游戏。祝您游戏愉快~'));
			}
...
function sel(){
	$sql= "SELECT * FROM `codednf` WHERE namer='1';";    //根据 Id值进行查询
    $stmt = link_mysql($sql);
    if($stmt->execute()) {
...
function add(){
	for ($x=0; $x<=10; $x++) {
		$sql= "INSERT INTO `codednf` VALUES (substring(MD5(RAND()),1,20) ,'1');"; // 根据ID值 将某字段修改为某参数
		$stmt = link_mysql($sql);
		$stmt->execute();
	}
	echo '成功创建十个激活码';
...
function up($name,$key){
    $sql= "UPDATE  `codednf` SET  namer =  '$name' WHERE  codekey='$key';"; // 根据ID值 将某字段修改为某参数
    $stmt = link_mysql($sql);
    if($stmt->execute()){
        return "修改成功！";
    }else{
